...
export class FirebaseTasirStore {
  ...
  async joinRoom(code:string,nickname:string){const normalized=code.trim().toUpperCase();const uid=await this.identity();const{db}=await getFirebaseServices();const roomRef=ref(db,`tasirRooms/${normalized}`);const snapshot=await get(roomRef);if(!snapshot.exists())throw new Error("Oda bulunamadı.");const initialRoom=snapshot.val();const result=await runTransaction(roomRef,(current)=>{const source=current??initialRoom;return joinTasirPlayer(normalizeTasirState(source),{uid,nickname});},{applyLocally:false});if(!result.committed){const latest=await get(roomRef);if(!latest.exists())throw new Error("Oda bulunamadı.");throw new Error("Odaya katılınamadı.");}return{room:normalizeTasirState(result.snapshot.val()),uid};}
